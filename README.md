...
Phase 0 は完了し、承認を受けて **Phase 1 の観測（2026-09-05 実捕捉）と Phase 2 の牌譜 decode を実装しました。**
`capture`、オフラインの `inspect`、牌譜再構築の `decode` が利用できます。MAKA 結合、匿名化保存、正規化モデル、MCP は未実装・未承認です。
...
- [ ] 半荘評価の取得元、局評価 rating のエンコード（候補 field / score 型 / 値域 1–99 は確認済み — [記録](docs/protocol-findings.md#phase1-live-capture)）
- [ ] score の方向、単純な差分に意味があるか、ランク表現
- [ ] 局 / 巡目 / action / decision の識別と join key（MAKA が全 seat を対象とすることは確認済み。`record_index` / `seer_index` は候補）
