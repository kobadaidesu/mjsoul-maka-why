...
雀魂の公式 AI「MAKA」は各打牌に評価値を付けてくれますが、**なぜその選択が悪いのかという理由は教えてくれません**。評価画面のスクリーンショットを LLM に読ませて解説させる手もありますが、牌姿・河・点数を画像から読み取る処理は複雑なわりに精度が出ず、実用になりませんでした。

## 解決

画像ではなく**データとして**渡せばいい、というのがこのツールです。ユーザー自身がブラウザで開いた牌譜と MAKA 表示の通信を**読み取り専用**で観測し、牌譜（盤面・全決断）を再構築して MAKA の評価値と結合し、MCP サーバとして LLM に提供します。LLM は正確な盤面と評価値をそのまま受け取れるので、「この打牌はなぜ悪いか」「MAKA の推奨とどれだけ差があるか」を高速かつ高い精度で解説できます。

```
牌譜 + MAKA 表示 ──(読み取り専用で観測)──> 再構築・結合 ──(MCP)──> LLM が理由を解説
```
...
既定では工程が分かる進行表示（`[ok] 接続` → `[..] スキャン中` → `[rx] 牌譜/MAKA 受信` → `[ok] 復元/保存`）を出します。表示するのは既存ログと同じ message 名・件数・保存パスのみで、payload や個人情報は含みません。従来の構造化ログが必要なら `--plain` を付けてください（各行に進行表示の分類用 `cli_event` 属性が付きます。固定の CLI 内部識別子で、payload・URL・個人情報は含みません）。
