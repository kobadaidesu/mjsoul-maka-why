...
<a id="call-actions"></a>
## 2026-09-06: 鳴き・特殊行動の action 値の実測

確認日時: 2026-09-06 17:25 JST。対象は Phase 1 capture の全 106 鳴き機会イベントと実際に起きた全 18 副露。
確認方法: 各副露（`RecordChiPengGang`）と直前の鳴き機会イベントの候補 action を突合し、さらに全鳴き機会の全予測について再構築手牌でその行動が物理的に可能かを全数検証した。

| action | 意味 | 根拠 |
| --- | --- | --- |
| 1 | 見送り（パス） | 全 99 出現が鳴き機会。鳴かなかったケースの最高 score と一致 |
| 2 | チー（下: 取った牌が順子の最小） | 実行 4/4 一致、予測 19 件すべて手牌で実行可能 |
| 3 | チー（中） | 実行 1/1 一致、予測 14 件すべて実行可能 |
| 4 | チー（上） | 実行 2/2 一致、予測 13 件すべて実行可能 |
| 5 | ポン | 実行 10/10 一致、予測 39 件すべて実行可能（赤5は通常牌 2 枚で成立するケース含む） |
| 6 | カン | ツモ後の暗槓/加槓（実行 1 件）に加え、他家打牌への明槓提示 1 件も手牌に刻子ありで整合 |
| 7 | 和了 | 既報（#phase3-maka-join）。鳴き機会側の 8 出現はロン提示で、実行された 1 件は直後に Hule |

矛盾 0（実行 18/18、予測フィージビリティ 86/86）。これらを `SeerCandidate.kind`（pass / chi_low / chi_mid / chi_high / pon / kan / win / discard / riichi_discard）として出力する。未実測の action 値は kind 空のまま raw を保持する。
チーの下/中/上の区別が 3 つの action 値に分かれること自体が、同一牌のチーでも喰い位置ごとに MAKA が別評価を持つことを意味する。
